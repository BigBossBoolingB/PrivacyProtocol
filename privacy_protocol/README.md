...
-   **Web Interface:** User-friendly interface for pasting text, viewing analysis, and managing preferences.
     *   **Policy History & Change Tracking (`/history`):**
         *   **Automatic History:** Every policy text you analyze is automatically saved (both the full analysis in `policy_history/` and an entry in `service_profiles.json` which tracks the latest analysis for each service).
         *   **View Policy History (`/history` page):** Navigate to the `/history` page to see an overview of the distinct services or policies you have analyzed. For each entry, it displays the service name (or identifier for pasted text), the date of its most recent analysis, its latest calculated `Service Risk Score` (0-100, color-coded), and a link to "View Details" for that specific historical snapshot.
     ...
     *   **Privacy Dashboard (`/dashboard`):** The dashboard provides a consolidated overview of your privacy posture based on all policies you've analyzed:
         ...
         *   **Key Privacy Insights:** The dashboard now presents dynamically generated insights based on your analyzed services. These insights can include warnings about services with high privacy risk scores, summaries of your overall risk distribution (e.g., if you have multiple medium-risk services), or general observations about your privacy posture. (Note: The range and sophistication of these insights will continue to evolve).
